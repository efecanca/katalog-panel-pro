...
void loginScreen(){
    ...
    void checkUpdate(){
        ...
        if(System.currentTimeMillis()-lastCheck < 21600000) return;
...
void home(){
            ...
            if(true){ // albumSendMode artık ayrı ekranda, burada her zaman yönetim
                ...
                if(photos.isEmpty()){
                    ...
                    for(int pi=0;pi<photos.size();pi++){
                        ...
                        String thumbUri=photos.get(pi);
                        boolean thumbIsPdf=thumbUri.toLowerCase().contains(".pdf");
                        if(!thumbIsPdf){
                            try{
                                String mt=getContentResolver().getType(Uri.parse(thumbUri));
                                thumbIsPdf=(mt!=null && mt.equals("application/pdf"));
                            }catch(Throwable ignored){}
                        }
                        if(thumbIsPdf){
                            imgBg.setColor(0xFFDC2626);
                            img.setScaleType(ImageView.ScaleType.CENTER);
                            img.setImageResource(android.R.drawable.ic_menu_save);
                            img.setColorFilter(Color.WHITE);
                        } else {
                            try{ img.setImageURI(Uri.parse(thumbUri)); }catch(Throwable ignored){}
                        }
    ...
    void cell(GridLayout grid,String uri){
        ...
        ImageView img=new ImageView(this); img.setScaleType(ImageView.ScaleType.CENTER_CROP);
        boolean cellIsPdf=uri.toLowerCase().contains(".pdf");
        if(!cellIsPdf){ try{ String mt=getContentResolver().getType(Uri.parse(uri)); cellIsPdf=(mt!=null && mt.equals("application/pdf")); }catch(Throwable ignored){} }
        if(cellIsPdf){
            img.setBackgroundColor(0xFFDC2626); img.setScaleType(ImageView.ScaleType.CENTER);
            img.setImageResource(android.R.drawable.ic_menu_save); img.setColorFilter(Color.WHITE);
        } else { try{img.setImageURI(Uri.parse(uri));}catch(Exception e){} }
    ...
    void galleryPicker(){ perms(false); Intent i=new Intent(android.provider.MediaStore.ACTION_PICK_IMAGES); i.putExtra(android.provider.MediaStore.EXTRA_PICK_IMAGES_MAX, 100); startActivityForResult(i,REQ_MEDIA); }
    void galleryPickerForAlbum(){ perms(false); Intent i=new Intent(android.provider.MediaStore.ACTION_PICK_IMAGES); i.putExtra(android.provider.MediaStore.EXTRA_PICK_IMAGES_MAX, 100); startActivityForResult(i,REQ_ALBUM_BASE+pickingAlbumIdx); }
...
// Tüm medyaları albüm olarak tek seferde gönder
    String uploadQueueV2(ArrayList<String> phonesList, ArrayList<String> fileUris,
...
// DISABLED_CRASH_FIX                     android.graphics.drawable.ClipDrawable clip=(android.graphics.drawable.ClipDrawable)subProgress.getProgressDrawable();
    ...
    void addOrMoveDialog(C c){
        ...
        if(phoneInSet(editingPhones,c.p)){
            ...
            if(adapter!=null) adapter.notifyDataSetChanged();
            updateCount();
            try{ cloudPushFavLists(); }catch(Exception ignored){}
            return;
        }
